...
function subscribeLastMessage(key, colRef, refreshTab) {
  ...
  lastMsgUnsubs[key] = colRef.orderBy('createdAt', 'desc').limit(1).onSnapshot(snap => {
    if (snap.empty) {
      ...
      const isFreshIncoming = !lastMsgFirstLoad[key] && senderId !== currentUser.uid;
      lastMsgCache[key] = {
  ...
  browserNotifToggleEl.onchange = async (e) => {
    ...
    try {
      ...
      if (perm === 'granted') {
        ...
        new Notification('Slygram', { body: 'Уведомления включены', tag: 'notif-test' });
      } else {
        e.target.checked = false;
        localStorage.setItem('browserNotif', '0');
...
function listenGroups(){
  unsubGroups = db.collection('groups')
    ...
    .onSnapshot(snap => {
      ...
      if (activeTab === 'groups') renderList('', !hasStructuralChange);
    });
}
...
function buildPersonRow(f, onClick) {
  ...
  row.innerHTML += `<div class="rowInfo"><div class="n">@${escapeHtml(f.username)} ${mutedTag}</div><div class="s${isOnline ? ' online' : ''}">${escapeHtml(subText)}</div></div>` +
...
function renderList(filter = '', skipAnim = false){
  ...
  if (activeTab === 'chats') {
    ...
    filtered.forEach(g => {
      ...
      const mutedTag = isChatMuted('g_' + g.id) ? `<span class="rowMuteIcon">${icon('bellOff', 12)}</span>` : '';
