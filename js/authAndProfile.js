  ...
  try {
    if (isLogin) {
      ...
      await db.collection('users').doc(uid).set({
        username, email, color, photo: null, banner: null, theme: 'paper', bio: '', channels: [], stickers: [], hideOnline: false,
        createdAt: firebase.firestore.FieldValue.serverTimestamp()
      });

      isRegistering = false;
      await enterApp(cred.user, username, color, null, null, 'paper', '', [], [], '', null, false);
    }
  } catch (e) {
...
function renderList(filter = ''){
  ...
  if (activeTab === 'chats') {
    ...
    filtered.forEach(f => list.appendChild(buildPersonRow(f, () => openChat(f))));
    ...
    if (filter && lastUserSearchResults.length > 0) {
      ...
      lastUserSearchResults.forEach(u => list.appendChild(buildPersonRow(u, () => openChat(u))));
    }
