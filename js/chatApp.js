...
async function enterApp(user, username, color, photo, theme, borderColor, bio, channels){
  currentUser = user;
  myUsername = username;
...
auth.onAuthStateChanged(async (user) => {
  if (user) {
    ...
    const d = doc.data();
    await enterApp(user, d.username, d.color, d.photo, d.theme, d.borderColor, d.bio, d.channels);
  } else {
...
function setupChatLayout(titleText, isGroup = false, targetObj = null) {
  ...
  if (canWrite) {
    ...
    // Предотвращаем потерю фокуса при клике тапом/мышкой, чтобы клавиатура на моб. не дергалась
    sendBtn.addEventListener('mousedown', (e) => e.preventDefault());
    sendBtn.addEventListener('touchstart', (e) => e.preventDefault());
