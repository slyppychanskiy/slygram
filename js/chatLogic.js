...
function openForwardPicker(msgData) {
  ...
  (myGroups || []).forEach(g => {
    ...
    const avContainer = document.createElement('div');
    avContainer.className = 'avatar';
    avContainer.innerHTML = icon(g.type === 'channel' ? 'megaphone' : 'users', 18);
    row.appendChild(avContainer);
    const nameEl = document.createElement('div');
    nameEl.className = 'fwdName';
...
function setupChatLayout(titleText, isGroup = false, targetObj = null) {
  ...
  if (!isGroup) {
    ...
    if (av) av.innerHTML = icon(targetObj.type === 'channel' ? 'megaphone' : 'users', 20);
    const hdrMain = document.querySelector('.hdrMain');
    if (hdrMain) {
      hdrMain.style.cursor = 'pointer';
      hdrMain.onclick = (e) => {
  ...
  function buildMessageEl(doc, animate) {
    ...
    // Один тап по всей строке сообщения (не только по самому пузырю) открывает
    // реакции + меню действий; двойной тап по той же строке ставит ❤️.
    // Открытие меню отложено на длительность окна двойного тапа: если
    // прилетает второй клик — это начало dblclick, и мы его гасим, чтобы
    // сердечко никогда не сопровождалось всплытием меню «ответить/закрепить/…».
    ...
    wrap.addEventListener('click', (e) => {
      ...
      msgTapTimer = setTimeout(() => {
        ...
        toggleEmojiBar(bar);
        if (actionsEl) actionsEl.classList.toggle('show');
      }, 260);
    });
