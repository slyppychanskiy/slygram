...
function setupChatLayout(titleText, isGroup = false, targetObj = null) {
  ...
  chatArea.innerHTML = `
    <div id="chatTopBar">
      <div id="chatHeader">
        ...
        <div>
          <button id="searchMsgToggleBtn" class="iconBtn chatHeaderIconBtn" title="Поиск в чате">🔍</button>
        </div>
      </div>
      <div id="chatSearchPanel">
        <input id="chatSearchInput" placeholder="Поиск в чате...">
        <button id="chatSearchClose" class="smallBtn">✕</button>
      </div>
      <div id="pinnedBar">
    ...
    <div id="inputBar">
      ...
      <div class="expressionWrapper">
        <button class="iconBtn" id="chatEmojiBtn" title="Эмодзи и стикеры">😊</button>
        <div id="expressionPanel" class="expressionPanel">
          ...
          <div id="stickerPane" class="expressionPane">
  ...
  function buildMessageEl(doc, animate) {
    ...
    let inner = '';
    if (m.forwarded) {
      inner += `<div class="forwardedTag">↪️ Переслано</div>`;
    }
    if (m.storyReply) {
    ...
    inner += `
      <div class="msgActions">
        ...
        ${(m.text || m.image || m.sticker || m.audio) ? '<button class="forwardBtn" title="Переслать">↪️</button>' : ''}
