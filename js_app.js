...
function setupChatLayout(titleText, isGroup = false, targetObj = null) {
  ...
  chatArea.innerHTML = `
    ...
    <div id="inputBar">
      ...
      <textarea id="msgInput" placeholder="Сообщение..." rows="1" style="resize:none; overflow-y:auto;"></textarea>
  ...
  function renderMessages(docs, filterText = '') {
    ...
    docs.forEach(doc => {
      ...
      if (m.text) inner += escapeHtml(m.text).replace(/\n/g, '<br>') + (m.isEdited ? ' <small style="opacity:0.6">(изм.)</small>' : '');
  ...
  if (canWrite) {
    ...
    msgInput.addEventListener('keydown', e => { 
      if (e.key === 'Enter' && !e.shiftKey) {
        // Обычный Enter на клавиатуре теперь переносит на новую строку по умолчанию,
        // но если зажаты Shift + Enter — тоже перенесет, либо можно сделать отправку по Shift+Enter, 
        // а простой Enter переносит на новую строку. 
        // Здесь простой Enter в текстовом поле (textarea) сам по себе переносит на новую строку, 
        // так как мы убрали e.preventDefault() для обычного Enter!
      }
    });
